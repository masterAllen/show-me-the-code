...
def read_xls(file_name):
    ...
    xls_sheet = xls_file.sheet_by_name('numbers')
    data = list()
    for i in range(xls_sheet.nrows):
        data.append(xls_sheet.row_values(i)[0:])
...
def save_to_xml(data, comment, new_file_name):
    ...
    child = xml.createElement('numbers')
...
content = read_xls('source/numbers.xls')
comment = '数字信息'
save_to_xml(content, comment, 'numbers.xml')
