...
# 参考: https://blog.csdn.net/shifanfashi/article/details/89448436
# 基本上是复制粘贴了，没办法，这个完全没接触过

def read_xls(file_name):
    # 1. Python 读取(excel/xls)文件
    xls_file = xlrd.open_workbook(file_name)
    xls_sheet = xls_file.sheet_by_name('student')
    data = {}
...
def save_to_xml(data, comment, new_file_name):
    ...
    child = xml.createElement('student')
...
content = read_xls('source/student.xls')
comment = '学生信息表 "id" : [名字, 数学, 语文, 英文]'
save_to_xml(content, comment, 'student.xml')
